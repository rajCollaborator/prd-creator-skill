...
Then, before adopting anything: **verify every reviewer claim against the code yourself.** Blind reviewers also overreach (recorded example: a claimed `isBankOnly` corruption risk where the flag turned out to be write-only). Adopt verified findings, reject the rest with the evidence, and record both in the decision log.
